...
net.cluster3d = function(l, filter = NULL){
  ...
  if (!is.null(filter)) {
    if (!is.numeric(filter) || length(filter) == 0 || any(filter <= 0) || any(filter != round(filter))) {
      stop("filter must be a vector of positive integers with length > 0.")
    }
  }
...
fix.line3d = function(x, tolerance, behaviour = 1, k = 1) {
  ...
  if(length(behaviour) != 1){stop("behaviour must be an integer from c(1, 2)")}
  if(!behaviour %in% c(1,2)){stop("behaviour must be an integer from c(1, 2)")}
  ...
  if (behaviour == 1) {
    ...
    Vlist = x %>% sf::st_coordinates() %>% as.data.frame() # Extract the vertex list
    ...
    st_crs(result) = sf::st_crs(x) # Define coordinate
    st_geometry(x) = sf::st_geometry(result) # Define coordinate
    return(x)  # <---- OUTPUT: sf LINESTRING feature
  }
  #->>> Behaviour 2
  else if (behaviour == 2) {
    # Convert x to network
    net = GISnetwork3D::sf_to_net3d(x)
    # Find the k neighbour
    snappedL = GISnetwork3D::knn3d(net$nodes, k = k) %>% subset(dist <= tolerance)
    ...
    if(nrow(snappedL) > 0){
      ...
      snappedL = GISnetwork3D::connect3d.pair(net$nodes[snappedL$id1,], net$nodes[snappedL$id2,])
      ...
      st_geometry(x)[newRow] = sf::st_geometry(snappedL) # Define coordinate
...
splitNearestV3d = function(x, y, restriction = NULL){
  ...
  if(!is.null(restriction)){
    if (!is.character(restriction) || length(restriction) != 1) {
      stop("restriction Must be a positive character.")}

    if (!restriction %in% names(y)) {
      stop("restriction is not available in y.")}

    restriction = y[[restriction]]

    if (!all(sort(unique(restriction)) %in% c(0, 1))) {
      stop("The values from y's restriction field must only be 0 and 1.")} }
...
blendPT3d = function(x,y, restriction = NULL){
  ...
  # Remove the duplicated vertices in segLine
  segLine$order = 1:nrow(segLine) # Record the order
  ends = cumsum(rle(segLine$ID)$lengths) # Cumulative end positions
  starts = c(1, head(ends, -1) + 1) # Start positions = end positions of previous group + 1
  segLine$nodePos = 0L # Initiate field to store node identifiers
  segLine$nodePos[starts] = 1L # Code start nodes as 1L
  segLine$nodePos[ends]  = 1L # Code end nodes as 1L
  segLine = segLine[segLine$nodePos == 1,] %>%
    rbind(segLine[segLine$nodePos == 0,] %>%
            dplyr::distinct(nodePos, ID, x, y, z, .keep_all = T)) # Remove duplicated vertices by each unique linestring
  segLine = segLine[order(segLine$order),] # Reorder the segLine
...
calCost.line3d = function(l, CF = "Campbell", pSlope = 8,
  ...
  if(length(CF) != 1){stop("CF must be a character of either `Tobler` or `Campbell.")}
  if(!CF %in% c("Tobler", "Campbell")){stop("CF must be a character of either `Tobler` or `Campbell.")}

  if(length(pSlope)!= 1){
    stop("pSlope must be a numeric value")
  }
  ...
  if(length(a)!= 1 || length(b)!= 1 || length(c)!= 1 || length(d)!= 1 || length(e)!= 1){
    stop("a, b, c, d, or e must be a numeric value for each.")}
...
dir.line3d = function(l, direction){
  ...
  if (length(direction) != nrow(l) || (!is.numeric(direction) & !is.integer(direction)) ) {
    stop("direction must be an integer vector and have the same length as l") }
...
sf_to_net3d = function(l){
  ...
  firstN = GISnetwork3D::getNode3d(l, sf = F, position = "first") # Extract first node
  endN = GISnetwork3D::getNode3d(l, sf = F, position = "last") # Extract last node
...
sf_to_net3d.full = function(l, direction = NULL, CF = "Campbell", force.pace = NULL, add.time = NULL, pSlope = 8,
  ...
  if(!is.null(direction)){
    if(!length(direction) == 1 || !is.character(direction)) {
      stop("direction must be a character with a length of 1") }
    if(!direction %in% names(l)){
      stop("Missing direction's field name from l") }

    direction = l[[direction]] }

  if(!is.null(direction)){
    if (length(direction) != nrow(l) || (!is.numeric(direction) & !is.integer(direction)) ) {
      stop("The values from l's direction field must only be -1, 0 and 1.") }}


  if(length(CF) != 1){stop("Invalid input for CF: Only accept a <character> from one of `Tobler`, `Campbell`.")}
  if(!CF %in% c("Tobler", "Campbell")){stop("CF must be a character from one of `Tobler` and `Campbell.")}

  if(!is.null(force.pace)){
    if(!length(force.pace) == 1 || !is.character(force.pace)) {
      stop("force.pace must be a character with a length of 1.") }
    if(!force.pace %in% names(l)){
      stop("Missing force.pace's field name from l.") }}

  if(!is.null(add.time)){
    if(!length(add.time) == 1 || !is.character(add.time)) {
      stop("add.time must be a character with a length of 1.") }
    if(!add.time %in% names(l)){
      stop("Missing add.time's field name from l.") }}
  ...
  l = calCost.line3d(l = l, CF = CF, pSlope = pSlope, a = a, b = b, c = c, d = d, e = e)
...
#' net_sub = subsetNet3d(net, v = c("n2", "n3", "n4"), exact = TRUE, output = "sf")
#' # Return as igraph object for network analysis:
#' g = subsetNet3d(net, v = c("n2", "n3", "n4"), exact = FALSE, output = "graph")
#' }
#' @export

subsetNet3d = function(net, v, exact = F, output = "sf"){

  # --- Input validation ---
  if (!inherits(net, "list") || !"nodes" %in% names(net) || !"edges" %in% names(net)) {
    stop("net must be a list containing 'nodes' and 'edges'.")}

  if(!is.character(v)){stop("v must be a character vector")}

  if(length(exact) != 1 & !is.logical(exact)){stop("exact must be a single logical value (TRUE/FALSE).")}

  if(length(output) != 1 || class(output) != "character"){ stop("output must be one of 'sf', 'df', or 'graph'.") }
  if (!output %in% c("sf", "df", "graph")) {stop("output must be one of 'sf', 'df', or 'graph'.")}
  ...
  if(output == "graph"){
    # Return igraph
    return(igraph::graph_from_data_frame(net$edges, directed = T, vertices = net$nodes))}  # <---- OUTPUT: igraph
  else if(output == "df"){
...
nearestNode3d = function(node_sf, pt){
  ...
  if (!inherits(node_sf, "sf") || !"sfc_POINT" %in% class(sf::st_geometry(node_sf))) {
    stop("node_sf must be an <sf> object with POINT geometry.") }

  if (!inherits(pt, "sf") || !"sfc_POINT" %in% class(sf::st_geometry(pt))) {
    stop("pt must be an <sf> object with POINT geometry.") }
...
#' l$direction = 0 # Assumes bi-directional
#'
#' # Create network
#' net = sf_to_net3d.full(l, direction = "direction", CF = "Tobler") # Assumes bi-directional
#'
#' # Find nearest nodes
...
nearestNode3d.rst = function(net, pt, restriction){
  ...
  if (!restriction %in% names(net$edges)) {
    stop("Missing restriction's field in the net's edges.")}

  restriction = net$edges[[restriction]]

  if (!all(sort(unique(restriction)) %in% c(0, 1))) {
    stop("restriction's field from y must be a integer vector with values of 0 and 1.")
  }
