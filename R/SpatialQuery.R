...
closestPT3d = function(x, y, output = "df"){
  ...
  if(is.character(output) == F | length(output) > 1){
    stop("Invalid inputs for output: Only accept ONE character")}

  if (!output %in% c("df", "sf")) {
    stop("Invalid input for output: Only accepts one of 'df' or 'sf'.")}
...
withinDist3d = function(x, y = NULL, dist){
  ...
  if(!is.null(y)){
    if (!inherits(y, "sf") ||
      ...
      stop("Invalid input for y: Only accept <sf POINT feature> OR <sf LINESTRING feature> if it is not NULL.")}

    if( "sfc_LINESTRING" %in% class(sf::st_geometry(x)) & "sfc_LINESTRING" %in% class(sf::st_geometry(y)) ){
      stop("Invalid input for x and y: Currently does not support line-to-line operation.")}
  }

  if(is.null(y)){
    if(!"sfc_POINT" %in% class(sf::st_geometry(x))){
      stop("Invalid input for x: Only accept <sf POINT feature> if y is null.")
    }
  }

  if (!is.numeric(dist) || length(dist) != 1 || dist < 0) {
    stop("Invalid input for dist: Only accepts ONE non-negative <numeric> value.")}
