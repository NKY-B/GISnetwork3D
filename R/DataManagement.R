...
addZ = function(x, y = NULL){
  ...
  if (!inherits(x, "sf") || (!"sfc_LINESTRING" %in% class(sf::st_geometry(x)) & !"sfc_POINT" %in% class(sf::st_geometry(x)))) {
    stop("Invalid input for x: Only accept <sf POINT feature> OR <sf LINESTRING feature>.") }

  if(!is.null(y)){
    if(!inherits(y, "RasterLayer")){stop("Invalid input for y: Only accept a <RasterLayer>.")}
  }
...
#' If \code{keep = TRUE}, all original attributes are associated with each segment; otherwise, only geometry is retained.
#'
...
segmentise3d = function(x, keep = FALSE){
  ...
  if (!is.logical(keep) || length(keep) != 1){
    stop("Invalid input for keep: Only accept one <logical> input of 'TRUE' OR 'FALSE'")}
...
getNode3d = function(x, sf = TRUE, position = "first"){
  ...
  if (!is.logical(sf) || length(sf) != 1){
    stop("Invalid input for sf: Only accept one <logical> input of 'TRUE' OR 'FALSE'")}

  if(length(position) != 1){
    stop("Invalid input for position: Only accept character of either `first` OR `last`")
  }

  if(!position %in% c("first", "last")){
    stop("Invalid input for position: Only accept character of either `first` OR `last`")
  }
...
#' Both input objects must have the same number of features.
#'
...
connect3d.pair = function(x, y){
  ...
  if (!inherits(y, "sf") || !"sfc_POINT" %in% class(sf::st_geometry(y))) {
    stop("Invalid input for y: Only accept <sf POINT feature>.") }

  if (nrow(x) != nrow(y)) {
    stop("Invalid inputs for x and y: Both must have the same number of features.") }
...
#'
#' # Points near, on, and far from the line
#' x = st_sf(geometry = st_sfc(
#'   st_point(c(2,1.2,1)),   # Near vertex
#'   st_point(c(4,0,2.5)),   # Near segment (projection)
#'   st_point(c(0,2,0)),     # Far from line
#'   st_point(c(8,0,4))      # On node
#' ))
#'
#' tolerance = 1.0
#'
...
snap3d.p2l = function(x, y, tolerance, rule = "node"){
  ...
  if (!rule %in% c("node", "vertex", "project")) {
    stop("Invalid input for rule: Only accepts one of 'node', 'vertex', or 'project'.")}
...
snap3d.l2l = function(x, y, tolerance, rule = "node"){
  ...
  if (!is.numeric(tolerance) || length(tolerance) != 1 || tolerance < 0) {
    stop("Invalid input for tolerance: Only accepts ONE non-negative <numeric> value.")}

  if(is.character(rule) == F | length(rule) > 1){
    stop("Invalid inputs for rule: Only accept ONE character")}
