...
dist3d = function(x,y){
  ...
  if("sfc_POINT" %in% class(sf::st_geometry(x)) & "sfc_POINT" %in% class(sf::st_geometry(y))){
    ...
    stop("Invalid input for x/y: Only accept <sf POINT feature> for x AND <sf POINT feature> OR <sf LINESTRING feature> for y.")
  }
}
...
dist3d.pair = function(x,y){
  ...
  if (!inherits(x, "sf") || !"sfc_POINT" %in% class(sf::st_geometry(x))) {
    stop("Invalid input for l: Only accept <sf POINT feature>.") }

  if ( !inherits(y, "sf") || !"sfc_POINT" %in% class(sf::st_geometry(y)) & !"sfc_LINESTRING" %in% class(sf::st_geometry(y)) ) {
    stop("Invalid input for y: Only accept <sf POINT feature> OR <sf LINESTRING feature>.") }

  if(nrow(x) != nrow(y)){
    stop("Invalid input for x and y: They must have same length.")
  }
...
#' line = st_sfc(st_linestring(matrix(c(0, 0, 1, 1, 2, 2), ncol = 2, byrow = TRUE)))
#'
#' # Convert to sf object
#' line_sf = st_sf(geometry = line)
#'
#' # Calculate slope in percent
#' slope_percent = calSlope(line_sf, unit = "percent")
#' print(slope_percent)
#'
#' # Calculate slope in degrees
#' slope_degrees = calSlope(line_sf, unit = "degrees")
#' print(slope_degrees)
#'
#' # Calculate slope in gradient
#' slope_gradient = calSlope(line_sf, unit = "gradient")
...
calSlope = function(l, unit){
  ...
  if (length(unit) != 1) {
    stop("Invalid input for unit: Only accept a single character from c('percent', 'degrees', 'gradient').")
  }

  if (!unit %in% c("percent", "degrees", "gradient")) {
    stop("Invalid input for unit: Only accept one of c('percent', 'degrees', 'gradient').")
  }
