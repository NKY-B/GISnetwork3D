...
#' @param o An \code{sf} object with 3D \code{POINT} geometry or a \code{character} vector of node IDs, representing origins. If \code{sf}, must include a column \code{nID} for the nearest network node.
#' @param d An \code{sf} object with 3D \code{POINT} geometry or a \code{character} vector of node IDs, representing destinations. If \code{sf}, must include a column \code{nID} for the nearest network node.
#' @param mode A \code{character} value specifying directionality: \code{"out"} (origin to destination), \code{"in"} (destination to origin), or \code{"both"} (sum of both directions).
...
#'
#' @return A \code{data.frame} representing the OD cost matrix, with rows corresponding to origins (\code{o}) and columns corresponding to destinations (\code{d}), containing \code{numeric} cost values based on \code{weight}.
#'
#' @examples
#' \dontrun{
#'
...
ODM3d = function(net, o, d, mode = "out", weight){
  # --- Input validation ---
  if (!inherits(net, "list") && !inherits(net, "igraph")) {
    stop("Invalid input for net: Only accept <igraph> OR a <list> of network.") }

  if (!is.character(weight) || length(weight) != 1) {
    stop("Invalid input for weight: Only accept ONE <character> value indicating the column name for weights.") }
  ...
  if (inherits(o, "sf") && inherits(d, "sf")) {
    ...
    if (!"nID" %in% colnames(o) || !"nID" %in% colnames(d)) {
      stop("Invalid input for o and/or d: <sf feature> must contain a column named 'nID'.")
    }
    ...
    stop("Invalid inputs for o and d: They must be <sf feature> OR <character vector>.")
  }
  ...
  if (inherits(net, "list")) {
    ...
    net = igraph::graph_from_data_frame(d = net$edges, directed = T, vertices = net$nodes %>% st_drop_geometry()) # Create igraph
  } else {
  ...
  if(mode == "out"){
    ...
    result = igraph::distances(net, v = o, to = unique.d, mode = "in", weights = weight)
    result = result + igraph::distances(net, v = o, to = unique.d, mode = "out", weights = weight)
  } else {stop("Invalid input for mode: Only accept one of <out>, <in>, or <both>.")}
...
access3d.pairOD = function(net, o, d, mode = "out", weight, threshold = 600, avaiW = NULL,
                           buf = 2000, nrows = 1, ncols = 2, ncores = 2){
  ...
  if (!mode %in% c("out", "in", "both")) {
    stop("Invalid input for mode: Only accepts one of 'out', 'in', or 'both'.")}

  if (!is.character(weight) || length(weight) != 1) {
    stop("Invalid input for weight: Only accepts ONE <character> value indicating the column name for weights.")}

  if (!is.numeric(threshold) || length(threshold) != 1 || threshold < 0) {
    stop("Invalid input for threshold: Only accepts ONE non-negative <numeric> value.")}

  if (!is.null(avaiW)) {
    if (!is.numeric(avaiW)) {
      stop("Invalid input for avaiW: Must be a <numeric vector>.")}
    if (length(avaiW) != nrow(d)) {
      stop("Invalid input for avaiW: Must have the same length as the number of destinations (`d`).")}}

  if (!is.numeric(buf) || length(buf) != 1 || buf < 0) {
    stop("Invalid input for buf: Only accepts ONE non-negative <numeric> value.")}

  if (!is.numeric(nrows) || length(nrows) != 1 || nrows < 0 || nrows %% 1 != 0) {
    stop("Invalid input for nrows: Only accepts ONE non-negative <integer> value.")}

  if (!is.numeric(ncols) || length(ncols) != 1 || ncols < 0 || ncols %% 1 != 0) {
    stop("Invalid input for ncols: Only accepts ONE non-negative <integer> value.")}

  if (!is.numeric(ncores) || length(ncores) != 1 || ncores <= 0 || ncores %% 1 != 0) {
    stop("Invalid input for ncores: Only accepts ONE positive <integer> value.")}

  # --- Automatic nearest node assignment ---
  ...
  if(nrows == 0 & ncols == 0){
    ...
    # Tile the origin and destination points
    OD = tile.OD(o, d, buf = buf, nrows = nrows, ncols = ncols)

    # Assign availability weights to each destinationn
    if(is.null(avaiW)){avaiW = rep(1, nrow(d))}
    for(i in 1:length(OD$d)){
      if(!is.null(OD$d[[i]]))
        OD$d[[i]]$avaiW = avaiW[OD$d[[i]]$id]}

    # Create an igraph object from the network
    net = igraph::graph_from_data_frame(d = st_drop_geometry(net$edges),
                                        directed = T, vertices = st_drop_geometry(net$nodes))

    # Compute accessibility using multiple cores
    future::plan("future::multisession", workers = ncores)
    result = furrr::future_map2( OD$o, OD$d,
                                 function(origin, destination, n, w, m, thred){
                                   if(is.null(destination)){
                                     return(NULL)
                                     } else {
                                       result = access3d.pairOD.sTask(net = n,
                                                                      o = origin$nID, d = destination$nID,
                                                                      mode = m, weight = w, threshold = thred,
                                                                      avaiW = destination$avaiW)
                                       result$oID = origin$id[result$oID]
                                       result$dID = destination$id[result$dID]
                                       return(result) }
                                   }, n = net, w = weight, m = mode, thred = threshold,
                                 .options = furrr::furrr_options(seed = TRUE, packages = c("igraph", "GISnetwork3D")))
...
##_____________________________________________________________________________
# -----> This function computes spatial accessibility with a single task.
# --> net = <igraph> OR a <list> of network: either an `igraph` object or a list containing nodes and edges.
# --> o = <character vector>: A vector of node names to indicate the origins.
# --> d = <character vector>: A vector of node names to indicate the destinations.
# --> mode = <character>: One of `c("out", "in", "both")` to indicate the direction of movement:
#     - `"out"`: Moving from the origin toward the destination.
#     - `"in"`: Moving back from the destination toward the origin.
#     - `"both"`: Summation of travel costs for both directions.
# --> weight = <character>: The column name in the network's edges defining the weight for calculating travel costs.
# --> threshold = <numeric>: Cost threshold within which the availability is computed (default is 600).
# --> avaiW = <numeric vector>: Weight of availability for each destination when computing the availability.
#     - Must have the same length as `d`. Default is `NULL`, meaning each destination is given a weight of 1.
...
access3d.pairOD.sTask = function(net, o, d, mode = "out", weight, threshold = 600, avaiW = NULL){
  ...
  if (!is.character(o)) {stop("Invalid input for o: Only accepts <character vector>.")}

  if (!is.character(d)) {stop("Invalid input for d: Only accepts <character vector>.")}

  if (!mode %in% c("out", "in", "both")) {stop("Invalid input for mode: Only accepts one of 'out', 'in', or 'both'.")}
  ...
  if (!is.null(avaiW)) {
    if (!is.numeric(avaiW)) {
      stop("Invalid input for avaiW: Must be a <numeric vector>.")}
    if (length(avaiW) != length(d)) {
      stop("Invalid input for avaiW: Must have the same length as `d`.")}
  }

  # --- Main function implementation ---
  # Calculate the ODM (Origin-Destination Matrix) using the ODM3d function
  ODM = GISnetwork3D::ODM3d(net = net, o = o, d = d, mode = mode, weight = weight)

  # Assign weights for availability computation if `avaiW` is NULL
  if (is.null(avaiW)) {
    avaiW = rep(1, length(d))  # Default weight is 1 for each destination
  }

  # --- Summarise the ODM ---
  # Create containers to store results
  oID = vector("integer", nrow(ODM))  # Origin ID
  oNode = vector("character", nrow(ODM))  # Origin node name
  dID = vector("integer", nrow(ODM))  # Destination ID
  dNode = vector("character", nrow(ODM))  # Destination node name
  costs = vector("numeric", nrow(ODM))  # Minimum travel costs
  avai = vector("numeric", nrow(ODM))  # Availability within cost threshold

  # Loop through each row of the ODM to summarise results
  for (i in 1:nrow(ODM)) {
    rowAttr = ODM[i, ]  # Subset the row from ODM
    oID[i] = i  # Assign origin ID
    oNode[i] = o[i]  # Assign origin node name
    costs[i] = min(rowAttr, na.rm = TRUE)  # Find the minimum travel cost
    dID[i] = which(rowAttr %in% costs[i])[1]  # Get the destination ID (only the first match)
    dNode[i] = d[dID[i]]  # Get the destination node name
    avai[i] = sum(avaiW[rowAttr <= threshold])  # Sum the availability within the cost threshold
  }
  # Return a data frame summarising the results
  return(data.frame(oID, oNode, dID, dNode, costs, avai))  # <---- OUTPUT: data.frame
}
...
#' @param tasks A positive \code{integer} value specifying the number of task chunks for parallel path summary computations (default: 2).
...
#'
#' @details
#' The function computes accessibility indicators for each 3D origin in \code{o}, including the nearest destination (by \code{weight}) and the weighted count of destinations within \code{threshold}, using \code{avaiW}. It also summarises edge attributes along each shortest path, supporting:
#' \itemize{
...
#' }
#' If \code{nID} is missing in \code{o} or \code{d}, the nearest node is assigned using \code{\link[GISnetwork3D]{nearestNode3d}}. Origins are divided into \code{nrows} x \code{ncols} spatial tiles, processing destinations within \code{buf} distance in parallel using \code{ncores} workers. If \code{nrows = 0} and \code{ncols = 0}, tiling is disabled. Path summaries are parallelised across \code{tasks} chunks.
#'
...
#' result = path3d.pairOD.summary(net, o, d, mode = "out", "time", threshold = 900, avaiW = c(1,1,5), weight = "time",
#'                                sumV = c("time", "energy"),
...
access3d.pairOD.summary = function(net, o, d, mode = "out", weight, threshold, avaiW = NULL,
                                   buf = 2000, nrows = 4, ncols = 4, ncores = 4, geom = TRUE, tasks = 2,
                                   sumV = NULL,
                                   w.sumV = NULL, w.sumW = NULL,
                                   p.sumV = NULL, pw.sumV = NULL, p.norm = NULL){

  # --- Compute access ---
  ODpair = GISnetwork3D::access3d.pairOD(net = net, o = o, d = d, mode = mode, weight = weight, threshold = threshold, avaiW = avaiW,
                                         buf = buf, nrows = nrows, ncols = ncols, ncores = ncores)


  # --- Summarise Path Attributes ---
  path.summary = GISnetwork3D::path3d.pairOD.summary(net = net, oNode = ODpair$oNode, dNode = ODpair$dNode,
                                                     mode = mode, weight = weight, oID = ODpair$oID, geom = geom, ncores = ncores, tasks = tasks,
                                                     sumV = sumV,
                                                     w.sumV = w.sumV, w.sumW = w.sumW,
                                                     p.sumV = p.sumV, pw.sumV = pw.sumV, p.norm = p.norm)

  # --- Combine accessibility and path summary results ---
  if(geom == FALSE){
    # Return a data.frame without geometry, joining accessibility and summary data by oID
    return(dplyr::left_join(ODpair, path.summary, by = "oID"))  # <---- OUTPUT: data.frame
  }
  if(geom == TRUE){
    # Return an sf object with geometry, merging accessibility and summary data
    names.ODpair = names(ODpair)
    names.path.summary = names(path.summary)
    ODpair = dplyr::left_join(path.summary, ODpair, by = "oID")
    # Ensure unique column names in the output to avoid duplication
    names.output = c(names.ODpair, names.path.summary) %>% unique()
    return(ODpair[names.output])
  }
}
...
path3d.pairOD.get_eID = function(net, oNode, dNode, mode = "out", weight, output = "list", oID = NULL){
  # --- Input validation ---
  if (!inherits(net, "list") && !inherits(net, "igraph")) {stop("Invalid input for net: Only accept <igraph> OR a <list> of network containing 'nodes' and 'edges'.")}

  if (!mode %in% c("out", "in")) {stop("Invalid input for mode: Only accepts one of 'out' or 'in'.")}

  if (!is.character(weight) || length(weight) != 1) {
    stop("Invalid input for weight: Only accepts ONE <character> value indicating the column name for weights.")}

  if (!is.character(oNode)) {stop("Invalid input for oNode: Must be a <character vector> specifying node indices.")}

  if (!is.character(dNode)) {stop("Invalid input for dNode: Must be a <character vector> specifying node indices.")}

  if (length(oNode) != length(dNode)) {stop("Invalid input for oNode and dNode: Both must have the same length.")}

  if (!is.character(output) || length(output) != 1){
    stop("Invalid input for output: Only accepts ONE <character> of 'list' or 'df'.")}
  ...
  if (!is.null(oID)) {
    if (length(oID) != length(oNode)) {
      stop("Invalid input for oID: Must have the same length as `oNode`.")}
    if (!is.numeric(oID) && !is.character(oID) && !is.integer(oID)) {
      stop("Invalid input for oID: Only accepts <integer>, <numeric>, or <character>.")
    }
  }
...
path3d.pairOD.getEdges = function(net, oNode, dNode, mode = "out", weight, oID = NULL, ncores = 4, tasks = 4, output = "raw"){
  ...
  if (!is.numeric(ncores) || length(ncores) != 1 || ncores <= 0 || ncores != round(ncores)) {
    stop("Invalid input for ncores: Must be a positive integer.")}

  if (!is.numeric(tasks) || length(tasks) != 1 || tasks <= 0 || tasks != round(tasks)) {
    stop("Invalid input for tasks: Must be a positive integer.")}

  if(is.null(oID)){oID = 1:length(oNode)}

  if(!is.character(output) || length(output) != 1){
    stop("Invalid input for output: Only accepts ONE <character> of 'eID', 'df', 'raw' or 'dissolve'.")
  }
...
path3d.pairOD.summary = function(net, oNode, dNode, mode = "out", weight, oID = NULL, geom = TRUE, ncores = 4, tasks = 4,
  ...
  if (length(oNode) != length(dNode)) {stop("Invalid inputs for oNode and dNode: Both must have the same length.")}
  ...
  if (!(weight %in% names(net$edges))) {stop(sprintf("weight column '%s' not found in net$edges.", weight))}
  ...
  if(!is.null(sumV) & !is.character(sumV)){stop("Invalid input for sumV: Only accept <NULL> OR <character vector>")}
  if(!is.null(w.sumV) & !is.character(w.sumV)){stop("Invalid input for w.sumV: Only accept <NULL> OR <character vector>")}
  if(!is.null(w.sumW) & !is.character(w.sumW)){stop("Invalid input for w.sumW: Only accept <NULL> OR <character>")}
  if(length(w.sumW) > 1){stop("Invalid input for w.sumW: Only accept ONE <character>")}
  if(!is.null(p.sumV) & !is.character(p.sumV)){stop("Invalid input for p.sumV: Only accept <NULL> OR <character vector>")}
  if(!is.null(pw.sumV) & !is.character(pw.sumV)){stop("Invalid input for pw.sumV: Only accept <NULL> OR <character vector>")}
  if(!is.null(p.norm) & !is.character(p.norm)){stop("Invalid input for p.norm: Only accept <NULL> OR <character>")}
  if(length(p.norm) > 1){stop("Invalid input for p.norm: Only accept ONE <character>")}

  if(!is.null(p.sumV) & is.null(p.norm)){stop("Invalid input for p.norm: Missing value")}
  if(!is.null(pw.sumV) & is.null(p.norm)){stop("Invalid input for p.norm: Missing value")}

  if( !is.null(p.norm) & ( is.null(p.sumV) & is.null(pw.sumV) ) ){stop("Invalid input for p.sumV: Missing value")}

  if(!is.null(w.sumV) & is.null(w.sumW)){stop("Invalid input for w.sumW: Missing value")}
  if(!is.null(w.sumW) & is.null(w.sumV)){stop("Invalid input for w.sumV: Missing value")}
...
#' road_network = GISnetwork3D::sf_to_net3d.full(road_network, direction = road_network$direction) # Activate network
#'
...
access3d.multitasks = function(startPT, taskPT, net, edges.output = NULL, weight, buf = 2000, nrows = 4, ncols = 4, ncores = 2, coef = 1.2){
  # --- Input validation ---
  if (!inherits(startPT, "sf") || !"sfc_POINT" %in% class(sf::st_geometry(startPT))) {
    stop("Invalid input for startPT: Only accept <sf POINT feature>.") }
  ...
  if (!is.list(taskPT) || any(!sapply(taskPT, function(x) inherits(x, "sf") && "sfc_POINT" %in% class(sf::st_geometry(x))))) {
    stop("Invalid input for taskPT: Only accept a <list> containing more than one <sf POINT> objects.")
  }

  if(any(!sapply(taskPT, function(x) "ID" %in% names(x)))){
    stop("Invalid input for taskPT: A field 'ID' is missing in the sf POINT obejct inside the list.")
  }
  ...
  if (!(is.null(edges.output) || (is.character(edges.output) && length(edges.output) == 1 && edges.output %in% c("dissolved", "raw", "raw.df")))) {
    stop("Invalid input for edges.output must be NULL or either from c('dissolved', 'raw', 'raw.df') .")
  }
  ...
  if (!is.numeric(coef) || length(coef) != 1) {
    stop("Invalid input for coef: Only accepts ONE <numeric> value.")}
  ...
  pts = GISnetwork3D::tile.multitasks.OD(o = startPT, d = taskPT, buf = buf, nrows = nrows, ncols = ncols, coef = coef)

  # --- Compute access for each startPT ---
  if(is.null(edges.output)){
    # Export result in data.frame
    # The data.frame includes start point ID, travel costs, ID of each stop, and nID (node ID) for each stops
    future::plan("future::multisession", workers = ncores) # Set up parallel processing
    # Compute the access for each tile
    result = furrr::future_map2(pts$o, pts$d, ~ GISnetwork3D::access3d.multitasks.sTask(startPT = .x, taskPT = .y,
                                                                                        net = net, weight = weight, export.IDs = T, export.nIDs = T),
                                .options = furrr::furrr_options(seed = TRUE, packages = c("igraph", "sf", "GISnetwork3D"))) %>% purrr::reduce(rbind)
    future::plan("future::sequential") # Stop parallel cluster

    result = result[match(startPT$ID, result$start.ID),] # Reorder the result to match the order of ID from the startPT
    return(result)  # <---- OUTPUT: data.frame
  }

  if(edges.output == "dissolved"){
    # Export result in sf LINESTRING object
    # The output includes start point ID, travel costs, ID of each stop, and nID (node ID) for each stops, and the dissolved geometry of the route for each path
    future::plan("future::multisession", workers = ncores) # Set up parallel plan
    # Compute the access for each tile
    result = furrr::future_map2(pts$o, pts$d, ~ GISnetwork3D::access3d.multitasks.sTask(startPT = .x, taskPT = .y,
                                                                                        net = net, weight = weight, export.IDs = T, export.nIDs = T, export.eID = T),
                                .options = furrr::furrr_options(seed = TRUE, packages = c("igraph", "sf", "GISnetwork3D")))
    future::plan("future::sequential") # Stop parallel cluster

    # Convert from list to data.frame for the access and the edge index
    result = list(access = purrr::map(result, ~.x$access) %>% purrr::reduce(rbind),
                  eID = purrr::map(result, ~.x$eID) %>% purrr::reduce(rbind))

    # Create the dissolve geometry for each path
    result$eID = cbind(result$eID$path.ID, dplyr::select(net$edges, "eID")[match(result$eID$eID, net$edges$eID),]) # Join edge geometry
    names(result$eID)[1] = "start.ID"
    result$eID = sfheaders::sf_to_df(result$eID, fill = T) # Convert from sf to data.frame
    result$eID = sfheaders::sf_linestring(obj = result$eID, x = "x", y = "y", z = "z", linestring_id = "start.ID", keep = T) %>% dplyr::select(start.ID) # Dissolve line
    sf::st_crs(result$eID) = sf::st_crs(net$edges)
    class(result$eID$start.ID) = class(startPT$ID)
    result = result$eID %>% dplyr::left_join(result$access, by = "start.ID")

    # Reorder the result to match the order of ID from the startPT
  ...
  if(edges.output == "raw"){
    # Export result in a list of 'access' and 'eID'
    # 'access' includes a data.frame including the start point ID, travel costs, ID of each stop, and nID (node ID) for each stops
    # 'eID' includes an sf LINESTRING object of all the edges traveled for each path. A field 'start.ID' is added to indicate the path ID.
    future::plan("future::multisession", workers = ncores) # Set up parallel plan
    # Compute the access for each tile
    result = furrr::future_map2(pts$o, pts$d, ~ GISnetwork3D::access3d.multitasks.sTask(startPT = .x, taskPT = .y,
                                                                                        net = net, weight = weight, export.IDs = T, export.nIDs = T, export.eID = T),
    ...
    # Extract the raw edges for each path
    result$eID = cbind(result$eID$path.ID, net$edges[match(result$eID$eID, net$edges$eID),])
    names(result$eID)[1] = "start.ID"

    # Reorder the access indicator according to the ID of the input point
    result$access = result$access[match(startPT$ID, result$access$start.ID),]


    # Reorder the edges according to the ID of the input point
    result$eID$edges.order = 1:nrow(result$eID) # Get the order of the edges
    result$eID = dplyr::left_join(result$eID, data.frame(start.ID = startPT$ID, startID.order = 1:nrow(startPT)), by = "start.ID") # Get the order of the ID from the original point
    result$eID = result$eID[order(result$eID$startID.order, result$eID$edges.order), ] # Order the edges according to the original ID and the sequence of the path
    result$eID = result$eID %>% dplyr::select(-edges.order, -startID.order)
    return(result)  # <---- OUTPUT: List
  }

  if(edges.output == "raw.df"){
    # Export result in a list of 'access' and 'eID'
    # 'access' includes a data.frame including the start point ID, travel costs, ID of each stop, and nID (node ID) for each stops
    # 'eID' includes a data.frame of all the edges traveled for each path. A field 'start.ID' is added to indicate the path ID.
    future::plan("future::multisession", workers = ncores) # Set up parallel plan
    # Compute the access for each tile
    result = furrr::future_map2(pts$o, pts$d, ~ GISnetwork3D::access3d.multitasks.sTask(startPT = .x, taskPT = .y,
                                                                                        net = net, weight = weight, export.IDs = T, export.nIDs = T, export.eID = T),
    ...
    result$eID = result$eID[order(result$eID$startID.order, result$eID$edges.order), ] # Order the edges according to the original ID and the sequence of the path
    result$eID = result$eID %>% dplyr::select(-edges.order, -startID.order)
    result$eID = sf::st_drop_geometry(result$eID)
    return(result)  # <---- OUTPUT: List
  }
...
access3d.multitasks.sTask = function(startPT, taskPT, net, weight, export.IDs = FALSE, export.nIDs = FALSE, export.eID = FALSE){

  # Create a data.frame of the total stops and clusters
  totalStop = taskPT %>% dplyr::distinct(cluster, nID, .keep_all = T) # Remove duplicated nID for each cluster

  ## Create OD matrix
  # ODM between all stops (Unique nID)
  OD.matrix = GISnetwork3D::ODM3d(net, o = unique(totalStop$nID), d = unique(totalStop$nID), mode = "out", weight = weight)
  rownames(OD.matrix) = unique(totalStop$nID)
  colnames(OD.matrix) = rownames(OD.matrix)
  diag(OD.matrix) = NA

  # ODM from start nodes to all stops
  OD.matrix.firstMile = GISnetwork3D::ODM3d(net, o = startPT$nID, d = unique(totalStop$nID), mode = "out", weight = weight)
  rownames(OD.matrix.firstMile) = startPT$nID
  colnames(OD.matrix.firstMile) = unique(totalStop$nID)

  # ODM from all stops to start nodes
  OD.matrix.lastMile = GISnetwork3D::ODM3d(net, o = unique(totalStop$nID), d = startPT$nID, mode = "out", weight = weight)
  rownames(OD.matrix.lastMile) = unique(totalStop$nID)
  colnames(OD.matrix.lastMile) = startPT$nID

  # For each input start node, find the best location from each task and derive the least-cost route
  result = purrr::map( split(startPT, 1:nrow(startPT)),
                function(start.Node, ODM, ODM.firstMile, ODM.lastMile, allStops){

                  # Create containers to store result
                  path.N = start.Node$nID
                  path.ID = NULL
                  path.C = NULL
                  fromN = start.Node$nID
                  finalCosts = 0

                  # Calculate the first mile costs
                  min.Cost = min(ODM.firstMile[fromN, ], na.rm = T) # Find the min cost
                  finalCosts = finalCosts + min.Cost # Update final costs

                  dNode = stats::na.omit( colnames(ODM.firstMile)[ODM.firstMile[fromN, ] == min.Cost] )[1] # Find the destination nodes
                  fromN = dNode # Update the start node for the next round

                  dCluster = allStops$cluster[ allStops$nID %in% dNode ] %>% unique() # Find the cluster that the destination belongs to
                  path.C = c(path.C, dCluster) # Record the sequence of cluster
                  path.N = c(path.N, rep(dNode, length(dCluster))) # Record the sequence of nodes
                  path.ID = c(path.ID, allStops$ID[ allStops$nID %in% dNode ]) # Record the sequence of ID

                  allStops = allStops[!allStops$cluster %in% path.C, ] # Remove stops of which the clusters are indexed
                  ODM = ODM[, colnames(ODM) %in% allStops$nID ] # Update the ODM

                  # Loop for each retaining cluster
                  for (i in 1:length(unique(allStops$cluster))){
                    if(nrow(allStops) > 0){
                      min.Cost = min(ODM[fromN, ], na.rm = T) # Find the min cost
                      finalCosts = finalCosts + min.Cost # Update final costs

                      dNode = stats::na.omit( colnames(ODM)[ODM[fromN, ] == min.Cost] )[1] # Find the destination nodes
                      fromN = dNode # Update the start node for the next found
                      ...
                      allStops = allStops[!allStops$cluster %in% path.C, ] # Remove stops of which the clusters are not indexed yet
                      ODM = ODM[, colnames(ODM) %in% allStops$nID, drop = F ] } } # Update the ODM

                  # Extract the cost for the last mile
                  finalCosts = finalCosts + ODM.lastMile[fromN, start.Node$nID]
                  path.N = c(path.N, start.Node$nID)

                  return(list(nodes = path.N, ID = path.ID, cluster = path.C, costs = finalCosts))
                }, ODM = OD.matrix, ODM.firstMile = OD.matrix.firstMile, ODM.lastMile = OD.matrix.lastMile, allStops = totalStop)

  output = data.frame(start.ID = startPT$ID, costs = unlist(map(result, ~.x$costs)))

  if(export.IDs == T){
    # Extract the unique ID of stops
    export.IDs = unlist(purrr::map(result, ~.x$ID)) # Extract the ID from stops
    export.IDs = matrix(export.IDs, ncol = length(taskPT$cluster %>% unique), byrow = T) # Convert to matrix
    colnames(export.IDs) = paste("ID.stop_", 1:length(taskPT$cluster %>% unique), sep = "") # Define column names
    output = output %>% cbind(export.IDs) # Join to the output
  }

  if(export.nIDs == T){
    # Extract the node ID (nID) of stops
    export.nIDs = unlist(purrr::map(result, ~.x$nodes)) # Extract the nID from stops
    export.nIDs = matrix(export.nIDs, ncol = length(taskPT$cluster %>% unique) + 2, byrow = T) # Convert to matrix
    colnames(export.nIDs) = c("nID.start", paste("nID.stop_", 1:length(taskPT$cluster %>% unique), sep = ""), "nID.end" ) # Define column names
    output = output %>% cbind(export.nIDs)
  }

  if(export.eID == T){
    input.nodes = data.frame( ID = rep(startPT$ID, unlist(purrr::map(purrr::map(result, ~.x$nodes), length))),
                              nID = unlist(map(result, ~ .x$nodes)) )
    output = list(access = output, eID = GISnetwork3D::path3d.multistops.get_eID(net = net, nodes = input.nodes, mode = "out",
                                                                                 weight = weight, output = "df"))
  }
  return(output)
...
#'   \item Weighted-normalised sum (\code{pw.sumV}, \code{p.norm}): Sum of (\code{pw.sumV} * \code{p.norm}) divided by sum of \code{p.norm} across route edges.
#' }
#' If \code{nID} is missing in \code{startPT} or \code{taskPT}, the nearest node is assigned using \code{\link[GISnetwork3D]{nearestNode3d}}. Spatial tiling (\code{nrows}, \code{ncols}, \code{buf}, \code{coef}) partitions data for parallel processing with \code{ncores} workers. The output includes one row per route, with columns for \code{start.ID} and requested summaries. If \code{geom = TRUE}, 3D \code{LINESTRING} geometries are included.
#'
#' @return An \code{sf} object with 3D \code{LINESTRING} geometry (if \code{geom = TRUE}) or a \code{data.frame} (if \code{geom = FALSE}) with columns:
#' \describe{
...
#'
#' # Compute the access that travel through the optimised location for each tasks
#' access3d.multitasks.summary(startPT = Origin_points_sf, taskPT = points_sf, net = road_network, edges.output = "dissolved", weight = "time", nrows = 4, ncols = 4, ncores = 2, sumV = c("time", "energy"))
#' }
#'
#' @export
access3d.multitasks.summary = function(startPT, taskPT, net, geom = FALSE, weight, buf = 2000, nrows = 1, ncols = 2, ncores = 2, coef = 1.2,
  ...
  if (!is.logical(geom) || length(geom) != 1){
    stop("Invalid input for geom: It must be TRUE or FALSE.")}
  ...
  # --- Compute shortest paths and extract edge attributes for each path ---
  if(isTRUE(geom)){
    vars = GISnetwork3D::access3d.multitasks(startPT = startPT, taskPT = taskPT, net = net, edges.output = "raw",
                                             weight = weight, buf = buf, nrows = nrows, ncols = ncols, ncores = ncores, coef = coef)

    output = vars$eID["start.ID"]
    output = output %>% sfheaders::sf_to_df(fill = T) # Convert sf to df
    output = output %>% sfheaders::sf_linestring(x = "x", y = "y", z = "z", linestring_id = "start.ID") # Group geometry by start.ID
    sf::st_crs(output) = sf::st_crs(net$edges)
    output = output %>% dplyr::left_join(vars$access, by = "start.ID")
    vars = vars$eID %>% sf::st_drop_geometry()
  }
  if(isFALSE(geom)){
    vars = GISnetwork3D::access3d.multitasks(startPT = startPT, taskPT = taskPT, net = net, edges.output = "raw.df",
                                             weight = weight, buf = buf, nrows = nrows, ncols = ncols, ncores = ncores, coef = coef)
    output = vars$access
    vars = vars$eID
  }

  # --- Prepare for attribute summarisation ---

  # Only keep relevant columns for summarisation
  vars = vars[c("start.ID", sumV, w.sumV, w.sumW, p.sumV, pw.sumV, p.norm) %>% unique()]

  # --- 1. Simple Sum ---
  if(!is.null(sumV)){
    sumV.df = vars[c("start.ID",sumV)] # Extract variables
    sumV.df = stats::aggregate(. ~ start.ID, FUN = sum, data = sumV.df) # Sum variables by start.ID
    output = dplyr::left_join(output, sumV.df, by = "start.ID") # Join to the output
  }

  # --- 2. Weighted Sum ---
  if(!is.null(w.sumV)){
    w.sumV.df = vars[c("start.ID",w.sumV)]  # Extract variables
    w.sumV.df[w.sumV] = w.sumV.df[w.sumV] * eval(parse(text = paste0("vars$", w.sumW)))  # Multiply variables by weight
    w.sumV.df = stats::aggregate(. ~ start.ID, FUN = sum, data = w.sumV.df) # Sum variables by start.ID
    names(w.sumV.df) = c("start.ID", paste("w.", w.sumV, sep = "")) # Add suffix for the variables
    output = output %>% dplyr::left_join(w.sumV.df, by = "start.ID") # Join to the output
  }

  # --- 3. Normalised Sum ---

  if(!is.null(p.sumV) & !is.null(p.norm)){
    p.sumV.df = vars[c("start.ID",p.sumV,p.norm)]  # Extract variables
    p.sumV.df = stats::aggregate(. ~ start.ID, FUN = sum, data = p.sumV.df) # Sum variables by start.ID
    p.sumV.df[p.sumV] = p.sumV.df[p.sumV] / eval(parse(text = paste0("p.sumV.df$", p.norm)))  # Normalise variables
    p.sumV.df = p.sumV.df[c("start.ID",p.sumV)]
    names(p.sumV.df) = c("start.ID", paste("p.", p.sumV, sep = ""))
    output = output %>% dplyr::left_join(p.sumV.df, by = "start.ID")
  }

  # --- 4. Weighted-Normalised Sum ---
  if(!is.null(pw.sumV) & !is.null(p.norm)){
    pw.sumV.df = vars[c("start.ID",pw.sumV,p.norm)]  # Extract variables
    pw.sumV.df[pw.sumV] = pw.sumV.df[pw.sumV] * eval(parse(text = paste0("pw.sumV.df$", p.norm)))  # Weight variables
    pw.sumV.df = aggregate(. ~ start.ID, FUN = sum, data = pw.sumV.df) # Sum variables by start.ID
    pw.sumV.df[pw.sumV] = pw.sumV.df[pw.sumV] / eval(parse(text = paste0("pw.sumV.df$", p.norm)))  # Normalise variables
    pw.sumV.df = pw.sumV.df[c("start.ID",pw.sumV)]
    names(pw.sumV.df) = c("start.ID", paste("pW.", pw.sumV, sep = ""))
    output = output %>% dplyr::left_join(pw.sumV.df, by = "start.ID")
  }

  return(output)  # <---- OUTPUT: data.frame OR sf LINESTRING feature
}
...
path3d.multistops.get_eID = function(net, nodes, mode = "out", weight, output = "list"){
  # --- Input validation ---
  if (!inherits(net, "list") && !inherits(net, "igraph")) {
    stop("Invalid input for net: Only accept <igraph> OR a <list> of network containing 'nodes' and 'edges'.")}

  if (!is.character(nodes) && !is.data.frame(nodes)) {
    stop("Invalid input for nodes: Must be a <character vector>, <sf POINT> object OR <data.frame>.")}

  if(is.data.frame(nodes)){
    if(!"ID" %in% names(nodes) || !"nID" %in% names(nodes)){
      stop("Invalid input for nodes: It must contain fields named 'ID' and 'nID'") } }

  if (!is.character(mode) || length(mode) != 1 || !mode %in% c("out", "in")) {
    stop("Invalid input for 'mode': must be one of 'out' or 'in'.")}

  if (!is.character(weight) || length(weight) != 1) {
    stop("Invalid input for weight: must be a single character string indicating the cost column.")}
  ...
  if (!is.character(output) || length(output) != 1 || !output %in% c("list", "df")) {
    stop("Invalid input for 'output': must be either 'list' or 'df'.") }
...
#' @param oID An \code{integer}, \code{numeric}, or \code{character} vector of unique origin identifiers, matching the length of \code{o}, or \code{NULL} (default: \code{1:nrow(o)}).
...
#'
#' @details
#' For each origin point in \code{o}, the function computes the set of nodes or edges reachable within the \code{threshold} cost (e.g., travel time or distance) along a 3D network, optionally constrained by a \code{searchDist} Euclidean cutoff. If \code{nID} is missing in \code{o}, the nearest node is assigned using \code{\link[GISnetwork3D]{nearestNode3d}}. The \code{mode} parameter determines directionality, with \code{"both"} minimising the sum of out-and-in costs. Outputs include node IDs, 3D point geometries, convex hull polygons, or 3D linestring geometries. Parallel processing splits origins into \code{tasks} chunks across \code{ncores} workers.
#'
#' @return Depends on \code{output}:
#' \describe{
...
#'
#' @export
catchment3d = function(o, net, mode, weight, threshold, searchDist, oID = NULL,
                       output = "ConvexHull", ncores = 4, tasks = 4){
  ...
  if (!is.numeric(searchDist) || length(searchDist) != 1 || searchDist < 0) {
    stop("Invalid input for searchDist: Only accepts ONE non-negative <numeric> value.")}

  if (!is.null(oID)) {
    if (length(oID) != nrow(o)) {
      stop("Invalid input for oID: Must have the same length as `o`.")}
    if (!is.numeric(oID) && !is.character(oID) && !is.integer(oID)) {
      stop("Invalid input for oID: Only accepts <integer>, <numeric>, or <character>.")}
  }

  if (!output %in% c("df", "point", "ConvexHull", "line")) {stop("Invalid input for output: Only accepts one of 'df', 'point', 'ConvexHull' or 'line'.")}

  if (!is.numeric(ncores) || length(ncores) != 1 || ncores <= 0 || ncores %% 1 != 0) {
    stop("Invalid input for ncores: Only accepts ONE positive <integer> value.")}

  if (!is.numeric(tasks) || length(tasks) != 1 || tasks <= 0 || tasks %% 1 != 0) {
    stop("Invalid input for tasks: Only accepts ONE positive <integer> value.")}
  ...
  # Define `oID` if not provided
  if (is.null(oID)) { oID = 1:nrow(o)} # Generate default IDs for origins

  # Combine oNode and oID
  input = data.frame(oNode = o$nID, oID = oID)
  input = split.data(input, tasks) # Split input into n tasks
  future::plan("future::multisession", workers = ncores) # Activate cores
  result = furrr::future_map(input,
                             ~ catchment3d.sCore(oNode = .x$oNode, net = net, mode = mode, weight = weight,
                                                 threshold = threshold, searchDist = searchDist,
                                                 oID = .x$oID, output = output),
...
catchment3d.sCore = function(oNode, net, mode, weight, threshold, searchDist, oID = NULL, output = "df"){

  # --- Search reachable nodes within the search distance ---
  o = net$nodes[match(oNode, net$nodes$nID), ] # Extract the origin nodes
  nodesWithin = sf::st_is_within_distance(o, net$nodes, dist = searchDist) # For each origin, find the network nodes within the search distance

  # --- Prepare network data for routing ---
  netG = igraph::graph_from_data_frame(d = net$edges, directed = T, vertices = net$nodes) # Create network
  weight = igraph::edge_attr(netG, weight) # Extract the weight
  if(is.null(oID)){oID = 1:length(oNode)} # If oID is not specific, a unique ID is assigned

  # --- Compute the catchment analysis ---
  # Create containder for storing results
  id = c() # origin ID
  nID = c() # IDs of nodes reachable
  eID = c() # IDs of edges reachable

  if(output == "line"){ # <------ Find eID if the output is line
    # For loop for each origin
    for(i in 1:length(nodesWithin)){
      dNodes = net$nodes$nID[ nodesWithin[[i]] ] # Find all destination nodes in sf point feature

      if(mode == "both"){ # <--- If mode == "both", sum the travel costs in both directions
        nodesReached = as.numeric(igraph::distances(netG, v = o$nID[i], to = dNodes, mode = "in", weights = weight ))
        nodesReached = nodesReached + as.numeric(igraph::distances(netG, v = o$nID[i], to = dNodes, mode = "out", weights = weight ))
        } else { # <--- If mode != "both", calculate the costs by direction
          nodesReached = igraph::distances(netG, v = o$nID[i], to = dNodes, mode = mode, weights = weight ) %>% as.numeric()
      }

      nodesReached = dNodes[nodesReached <= threshold] # Index nodes within the threshold costs
      edgesReached = net$edges$eID[net$edges$FROM %in% nodesReached & net$edges$TO %in% nodesReached] # Extract eID of which both FROM and TO contain the indexed nID
      eID = c(eID, edgesReached) # Join to the result
      id = c( id, rep(oID[i], length(edgesReached)) ) }  # Assign id
  }
  else { # <------ Find oID if the output is NOT line
    for(i in 1:length(nodesWithin)){
      dNodes = net$nodes$nID[ nodesWithin[[i]] ] # Find all destination nodes in sf point feature
      ...
      nodesReached = dNodes[nodesReached <= threshold] # Index nodes within the threshold costs
      nID = c(nID, nodesReached) # Join to the result
      id = c( id, rep(oID[i], length(nodesReached)) )  }  # Assign id
  }

  if(output == "df"){ # <------ Output as data.frame
    return( data.frame(id = id, nID = nID))}  # <---- OUTPUT: data.frame
  else if(output == "point"){ # <------ Output as sf point
    result = data.frame(id = id, nID = nID)
    result = cbind( result, sf::st_coordinates(net$nodes[match(result$nID, net$nodes$nID),]) )
    result = sfheaders::sf_multipoint(result, x = "X", y = "Y", z = "Z", multipoint_id = "id", keep = T)
    st_crs(result) = st_crs(net$nodes)
    return(result["id"])}  # <---- OUTPUT: sf POINT feature
  else if(output == "ConvexHull"){ # <------ Output as sf polygon of Convex Hull
    result = data.frame(id = id, nID = nID)
    result = cbind( result, sf::st_coordinates(net$nodes[match(result$nID, net$nodes$nID),]) )
    result = sfheaders::sf_multipoint(result, x = "X", y = "Y", z = "Z", multipoint_id = "id", keep = T)
    st_crs(result) = st_crs(net$nodes)
    result = st_convex_hull(result)["id"]}  # <---- OUTPUT: sf POLYGON feature
  else if(output == "line"){ # <------ Output as sf MULTILINESTRING feature
    result = data.frame(id = id, eID = eID)
    result = cbind(id, net$edges[match(result$eID, net$edges$eID),] %>% dplyr::select())
    result = sfheaders::sf_to_df(result, fill = T)
    result = sfheaders::sf_multilinestring(result[c("id", "x", "y", "z", "linestring_id")], x = "x", y = "y", z = "z",
                                           linestring_id = "linestring_id", multilinestring_id = "id", keep = T)
    st_crs(result) = st_crs(net$nodes)
    return(result)  # <---- OUTPUT: sf MULTILINESTRING feature
  }
